...
async def gcast(event):
    ...
    kk = await edit_or_reply(event, "`Sabar Ngentod Lagi Gcast...`")
    er = 0
    done = 0
        ...
        if x.is_group:
            ...
            if chat not in GCAST_BLACKLIST and chat not in BLACKLIST_GCAST:
                try:
                    ...
                    er += 1
    await kk.edit(
        f"**Sukses Nyasar Ke** `{done}` **Grup, Gagal Nyasar Ke Ke** `{er}` **Grup**"
    )
        ...
        if x.is_user and not x.entity.bot:
            ...
            if chat not in DEVS:
                try:
                    ...
                    er += 1
    await kk.edit(
        f"**Sukses Nyasar Ke** `{done}` **chat, Gagal Nyasar Ke** `{er}` **chat**"
    )
